...
export function useCustomerDashboard() {
  ...
  const currentUserAddress = useMemo(() => {
    ...
    return `${activeAddress.name || ""} ${activeAddress.phone || ""} ${activeAddress.street || ""} ${activeAddress.city || ""} ${activeAddress.state || ""}, ${activeAddress.pincode || ""} `;
  }, [activeAddress]);
  ...
  }, [currentUser?._id, dispatch, openModal]);

  // Helper function for title formatting with useCallback
  ...
  // Helper function for price formatting with useCallback
  const getOrderPrice = useCallback((order) => {
  ...
  }, [totalOrdersCount, recentOrders, currentUser?.myWishlist]);

  const pastTwoOrders = useMemo(() => {
    return recentOrders.slice(0, 2);
  }, [recentOrders]);
